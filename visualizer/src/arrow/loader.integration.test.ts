 ...
 * own round-trip self-test (17 events, re-parents at T+84 h and T+120 h).
 *
...
describe.skipIf(!hasFixture)("dummy.arrows fixture", () => {
  it("loads every row of every entity", () => {
    ...
    // + 5 demo entities hourly.
    expect(data.rows.length).toBe(8 * 15 + 2 * 169 + 5 * 169);
    expect(data.entities.size).toBe(15);
  ...
  it("resolves every id to a registered name", () => {
    ...
    expect(data.names.isAstronomical(idOf("demo:rover-1"))).toBe(false);
  });
  ...
  it("keeps native units per entity (store raw, reproject on demand)", () => {
    ...
    expect(unitsOf("demo:moon-base-1")).toEqual(new Set(["m"]));
    expect(unitsOf("demo:rover-1")).toEqual(new Set(["m"]));
    expect(unitsOf("demo:miner-1")).toEqual(new Set(["km", "mm"]));
  });

  it("re-derives the same 17 topology events as the Rust ledger", () => {
    expect(topology().events.length).toBe(17);
  });

  it("resolves the rover's three-deep chain through the base", () => {
    const { data, idOf } = fixture();
    const chain = topology().chainAt(idOf("demo:rover-1"), t0 + 12n * NS_PER_HOUR);
  ...
  it("keeps the base stationary in the Moon's frame", () => {
    const { data, idOf } = fixture();
    const rows = data.entities.get(idOf("demo:moon-base-1"))!;
    // A surveyed installation does not move relative to the body it sits on;
    // the Moon's own rows carry the rotation on its behalf.
    for (const r of rows) expect(r.position).toEqual(rows[0]!.position);
    expect(Math.hypot(...rows[0]!.position)).toBeCloseTo(1_737_400, 0); // metres
  });
  ...
  it("decodes kinematics: real body velocities, rover angular velocity", () => {
    const { data, idOf } = fixture();
    const earth = data.entities.get(idOf("Earth"))!;
    expect(earth.every((r) => r.velocity !== null)).toBe(true);
    const rover = data.entities.get(idOf("demo:rover-1"))!;
    expect(rover[0]!.angularVelocity).not.toBeNull();
    // The rover is a child of the base, not of the Moon directly.
    expect(rover[0]!.frameId).toBe(idOf("demo:moon-base-1"));
    // ...and the base's frame IS the Moon's id — a body is its own body-fixed frame.
    expect(data.entities.get(idOf("demo:moon-base-1"))![0]!.frameId).toBe(idOf("Moon"));
  });
  ...
  it("carries real ephemeris, not a synthetic circle", () => {
    ...
    // would leave the rover on a Moon that never rotates.
